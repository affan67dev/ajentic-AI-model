...
import { ArrowUp, AudioLines, Check, Command, Copy, FileUp, Menu, MoreHorizontal, Plus, Search, Settings2, Share2, Sparkles, ThumbsDown, ThumbsUp, Volume2, X, Zap } from 'lucide-react'
...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <div className="composer-wrap"><div className="composer"><textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey && !e.nativeEvent.isComposing && e.keyCode !== 229) { e.preventDefault(); sendMessage() } }} placeholder="Message your AI..." rows={1} /><div className="composer-footer"><button className="attach-btn" aria-label="Attach file"><FileUp size={18} /></button><span className="composer-hint">Shift + Enter for new line</span><div className="composer-tools"><button aria-label="Voice input"><AudioLines size={18} /></button><button className={`send-btn ${input.trim() ? 'send-active' : ''}`} onClick={sendMessage} aria-label="Send message"><ArrowUp size={18} /></button></div></div></div><div className="composer-note"><Zap size={12} /> OmniBrain can make mistakes. Check important information.</div></div><nav className="bottom-nav" aria-label="App navigation"><button className="nav-item nav-active" aria-label="Home"><Sparkles size={15} /></button><button className="nav-item" onClick={newChat} aria-label="New chat"><Plus size={17} /></button><button className="nav-item" onClick={() => setDrawerOpen(true)} aria-label="Chats"><Search size={15} /></button></nav>
    </section>
  </main>
}
