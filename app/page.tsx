...
function renderMessageContent(content: string) {
  ...
  content.split('\n').forEach((line, index) => {
    ...
    const className = line.startsWith('### ') ? 'message-heading' : line.startsWith('- ') ? 'bullet-line' : undefined
    blocks.push(<p className={className} key={`line-${index}`}>{line.startsWith('### ') ? line.slice(4) : line.startsWith('- ') ? line.slice(2) : line}</p>)
  })
